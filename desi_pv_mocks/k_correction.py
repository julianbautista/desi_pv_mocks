...
class GAMA_KCorrection(KCorrection):
    ...
    Args:
        cosmology: object of type astropy.cosmology
        k_corr_file: file of polynomial coefficients for each colour bin
        z0: reference redshift [default: 0.1]
        cubic_interpolation: if set to True, will use cubic spline interpolation.
                             if False, will use linear interpolation
                             [default: True]
    """
    def __init__(self, cosmology, k_corr_file, z0=0.1, cubic_interpolation=True):
        
        # read file of parameters of polynomial fit to k-correction
        cmin, cmax, A, B, C, D, E, cmed = np.loadtxt(k_corr_file, unpack=True)
    ...
    def apparent_magnitude(self, absolute_magnitude, redshift, colour):
        ...
        """
        # Luminosity distance
        D_L = (1.+redshift) * self.cosmo.comoving_distance(redshift).value * self.cosmo.h
...
if __name__ == "__main__":
    ...
    # plot the r-band and g-band k-corrections
    ...
    # files of polynomial coefficients
    k_corr_file_r = 'k_corr_rband_z01.dat'
    k_corr_file_g = 'k_corr_gband_z01.dat'

    # arrays of redshift and colour
    z = np.arange(0,0.5,0.01)
    col = np.arange(0.1, 1.11, 0.1)
    
    # plot r-band k-correction
    k_r = GAMA_KCorrection(cosmology, k_corr_file_r)

    for i in range(len(col)):
        c = np.ones(len(z)) * col[i]
        plt.plot(z, k_r.k(z, c), label=r"$^{0.1}(g-r)=%.2f$"%col[i])
    plt.xlabel("z")
    plt.ylabel(r"$k_r(z)$")
    plt.legend(loc="upper left", fontsize=8).draw_frame(False)
    plt.show()
    

    # plot g-band k-correction
    k_g = GAMA_KCorrection(cosmology, k_corr_file_g)

    for i in range(len(col)):
        c = np.ones(len(z)) * col[i]
        plt.plot(z, k_g.k(z, c), label=r"$^{0.1}(g-r)=%.2f$"%col[i])
    plt.xlabel("z")
    plt.ylabel(r"$k_g(z)$")
    plt.legend(loc="upper left", fontsize=8).draw_frame(False)
    plt.show()
