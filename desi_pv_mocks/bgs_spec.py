...
def load_and_clean_spec(k_r):
    ...
    log.info(f"Loading spec catalogue from {CONFIG.spec_csv}")
    spec = pd.read_csv(CONFIG.spec_csv, usecols=CONFIG.spec_keys)
    log.info("  %d rows before cuts", len(spec))
 
    #spec = spec[spec["deltachi2"] >= BGS_SPEC.deltachi2_min]
    spec = spec[spec["zwarn"] == 0]
    spec = spec[spec["z"] <= BGS_SPEC.zmax]
    spec = spec[(spec["flux_g"] > 0) & (spec["flux_r"] > 0) & (spec["flux_z"] > 0)]
    spec["col"] = spec["mag_g"] - spec["mag_r"]
    spec = spec[(spec["col"] >= BGS_SPEC.col_min) & (spec["col"] <= BGS_SPEC.col_max)]
    spec["abs_mag_r"] = k_r.absolute_magnitude(spec["mag_r"], spec["z"], spec["col"])
    spec = spec[spec["mag_r"] <= BGS_SPEC.r_mag_lim]
...
def main():
    ...
    log.info("=== Cross-matching mocks ===")
    os.makedirs(CONFIG.mock_bgs_spec_dir, exist_ok=True)
 
    for phase in range(CONFIG.n_phases):
        for real in range(CONFIG.n_reals):
            mock_infile  = CONFIG.mock_bgs_base_data.format(phase=phase, real=real) 
            mock_outfile = CONFIG.mock_bgs_spec_data.format(phase=phase, real=real)
    
            if os.path.exists(mock_outfile) and not BGS_SPEC.overwrite:
    ...
    # ------------------------------------------------------------------
    if BGS_SPEC.do_diagnostic_plots:
        os.makedirs(CONFIG.mock_bgs_spec_plot_dir, exist_ok=True)
        ...
        plot_color_magnitude(
            ...
            CONFIG.mock_bgs_spec_plot_dir / "BGS_PV_AbacusSummit_M_vs_gr.png",
        )
        plot_mass_ssfr(
            spec, counts_msfr, massbins, ssfrbins,
            CONFIG.mock_bgs_spec_plot_dir / "BGS_PV_AbacusSummit_logM_vs_sSFR.png",
        )
